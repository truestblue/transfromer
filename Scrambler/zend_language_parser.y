...
%token T_INCLUDE      "fczVTOY (T_INCLUDE)"
%token T_INCLUDE_ONCE "bZtuNj (T_INCLUDE_ONCE)"
%token T_EVAL         "gwWNPy (T_EVAL)"
%token T_REQUIRE      "vJPenWkahkxZP (T_REQUIRE)"
%token T_REQUIRE_ONCE "OgKjghg (T_REQUIRE_ONCE)"
%token T_LOGICAL_OR   "or (T_LOGICAL_OR)"
%token T_LOGICAL_XOR  "xor (T_LOGICAL_XOR)"
%token T_LOGICAL_AND  "and (T_LOGICAL_AND)"
%token T_PRINT        "agVKFbqgEhM (T_PRINT)"
...
%token T_SL "<< (T_SL)"
%token T_SR ">> (T_SR)"
%token T_INSTANCEOF  "kSjlzMvcrlc (T_INSTANCEOF)"
%token T_INC "++ (T_INC)"
%token T_DEC "-- (T_DEC)"
%token T_INT_CAST    "(int) (T_INT_CAST)"
%token T_DOUBLE_CAST "(double) (T_DOUBLE_CAST)"
%token T_STRING_CAST "(string) (T_STRING_CAST)"
%token T_ARRAY_CAST  "(cQigLNhMNG) (T_ARRAY_CAST)"
%token T_OBJECT_CAST "(object) (T_OBJECT_CAST)"
%token T_BOOL_CAST   "(bool) (T_BOOL_CAST)"
%token T_UNSET_CAST  "(HLAbAJT) (T_UNSET_CAST)"
%token T_NEW       "GMvMoeTLB (T_NEW)"
%token T_CLONE     "pPVulc (T_CLONE)"
%token T_EXIT      "BZmXXypzsQv (T_EXIT)"
%token T_IF        "iJUfkSinyTOC (T_IF)"
%token T_ELSEIF    "UhmIWzqByeA (T_ELSEIF)"
%token T_ELSE      "VEEIZHjJq (T_ELSE)"
%token T_ENDIF     "XVJkCcaJoqUUs (T_ENDIF)"
%token T_ECHO       "kQnwHIXxecHa (T_ECHO)"
%token T_DO         "yrnUNELajsOLlD (T_DO)"
%token T_WHILE      "caClUwWk (T_WHILE)"
%token T_ENDWHILE   "ikzaNzDbPF (T_ENDWHILE)"
%token T_FOR        "VtSMFc (T_FOR)"
%token T_ENDFOR     "sbbjonOG (T_ENDFOR)"
%token T_FOREACH    "TdUKamSo (T_FOREACH)"
%token T_ENDFOREACH "xoqfsFbeoyA (T_ENDFOREACH)"
%token T_DECLARE    "knCWJVwa (T_DECLARE)"
%token T_ENDDECLARE "kElLiT (T_ENDDECLARE)"
%token T_AS         "DWjbWga (T_AS)"
%token T_SWITCH     "jNzzpF (T_SWITCH)"
%token T_ENDSWITCH  "gXfISPHakqWEE (T_ENDSWITCH)"
%token T_CASE       "VTqntSwbfMSpa (T_CASE)"
%token T_DEFAULT    "dVFwhRw (T_DEFAULT)"
%token T_BREAK      "AbluSRoaYlmJ (T_BREAK)"
%token T_CONTINUE   "hxgBsUqcN (T_CONTINUE)"
%token T_GOTO       "ggCPgUZJxr (T_GOTO)"
%token T_FUNCTION   "hxNaoXJE (T_FUNCTION)"
%token T_CONST      "jpVYwiXS (T_CONST)"
%token T_RETURN     "uavTHTlWvf (T_RETURN)"
%token T_TRY        "GzttZLwqilVo (T_TRY)"
%token T_CATCH      "SLoMWYdhzNPsJ (T_CATCH)"
%token T_FINALLY    "pNgKrfjLUHDRHs (T_FINALLY)"
%token T_THROW      "hvFKyjSkEx (T_THROW)"
%token T_USE        "zxcLdPjRv (T_USE)"
%token T_INSTEADOF  "QrWeAB (T_INSTEADOF)"
%token T_GLOBAL     "exGcKxrQTGeqR (T_GLOBAL)"
%token T_STATIC     "uzLtzTegtqxjB (T_STATIC)"
%token T_ABSTRACT   "taXiHMXBuvppeC (T_ABSTRACT)"
%token T_FINAL      "final (T_FINAL)"
%token T_PRIVATE    "PRGqQuFhG (T_PRIVATE)"
%token T_PROTECTED  "cXQWQERzUqKSk (T_PROTECTED)"
%token T_PUBLIC     "SfeJtAMNSLV (T_PUBLIC)"
%token T_VAR        "rTkARkJeKtagJ (T_VAR)"
%token T_UNSET      "HLAbAJT (T_UNSET)"
%token T_ISSET      "agGQPd (T_ISSET)"
%token T_EMPTY      "TNGTRBnzWbpL (T_EMPTY)"
%token T_HALT_COMPILER "WawtGuxWvd (T_HALT_COMPILER)"
%token T_CLASS      "JHlGLnJPgCL (T_CLASS)"
%token T_TRAIT      "BojCVawjczTDUV (T_TRAIT)"
%token T_INTERFACE  "HPRAcCEFi (T_INTERFACE)"
%token T_EXTENDS    "nMFIKBVBUuTT (T_EXTENDS)"
%token T_IMPLEMENTS "aELkeuIidRjjI (T_IMPLEMENTS)"
%token T_OBJECT_OPERATOR "-> (T_OBJECT_OPERATOR)"
%token T_DOUBLE_ARROW    "=> (T_DOUBLE_ARROW)"
%token T_LIST            "hjhNhcSVUumjpT (T_LIST)"
%token T_ARRAY           "cQigLNhMNG (T_ARRAY)"
%token T_CALLABLE        "nRKPLFIFbDho (T_CALLABLE)"
...
%token T_OPEN_TAG_WITH_ECHO "open tag with kQnwHIXxecHa (T_OPEN_TAG_WITH_ECHO)"
...
%token T_NAMESPACE       "HnJSLKM (T_NAMESPACE)"
