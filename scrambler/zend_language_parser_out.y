...
%token T_INCLUDE      "CzOfTOr (T_INCLUDE)"
%token T_INCLUDE_ONCE "KGrhYQ (T_INCLUDE_ONCE)"
%token T_EVAL         "lcufii (T_EVAL)"
%token T_REQUIRE      "ykjEnGGNjzMAw (T_REQUIRE)"
%token T_REQUIRE_ONCE "iPczzIv (T_REQUIRE_ONCE)"
%token T_LOGICAL_OR   " (T_LOGICAL_OR)"
%token T_LOGICAL_XOR  " (T_LOGICAL_XOR)"
%token T_LOGICAL_AND  " (T_LOGICAL_AND)"
%token T_PRINT        "HthDVUqfxdC (T_PRINT)"
...
%token T_SL "<< (T_SL)"
%token T_SR ">> (T_SR)"
%token T_INSTANCEOF  "VshpxLhKhLO (T_INSTANCEOF)"
%token T_INC "++ (T_INC)"
%token T_DEC "-- (T_DEC)"
%token T_INT_CAST    "(int) (T_INT_CAST)"
%token T_DOUBLE_CAST "(double) (T_DOUBLE_CAST)"
%token T_STRING_CAST "(string) (T_STRING_CAST)"
%token T_ARRAY_CAST  "(leKZdPRncy) (T_ARRAY_CAST)"
%token T_OBJECT_CAST "(object) (T_OBJECT_CAST)"
%token T_BOOL_CAST   "(bool) (T_BOOL_CAST)"
%token T_UNSET_CAST  "(TFSlkXy) (T_UNSET_CAST)"
%token T_NEW       "uodvtDXwY (T_NEW)"
%token T_CLONE     "KUFbSE (T_CLONE)"
%token T_EXIT      "yfPeZWxMPZB (T_EXIT)"
%token T_IF        "lPlzHxBdWyPE (T_IF)"
%token T_ELSEIF    "ooLAaaqCKxs (T_ELSEIF)"
%token T_ELSE      "IxtMJEXyX (T_ELSE)"
%token T_ENDIF     "XQLjDhgIPdURi (T_ENDIF)"
%token T_ECHO       "LMsvwqWMjeGR (T_ECHO)"
%token T_DO         "mLRmIFlqBIZArV (T_DO)"
%token T_WHILE      "uvyMBKye (T_WHILE)"
%token T_ENDWHILE   "FEcMRmxmfL (T_ENDWHILE)"
%token T_FOR        "WwGdYG (T_FOR)"
%token T_ENDFOR     "OUVMYCRb (T_ENDFOR)"
%token T_FOREACH    "VNuESmaJ (T_FOREACH)"
%token T_ENDFOREACH "HtcPKilTVtu (T_ENDFOREACH)"
%token T_DECLARE    "ybpsncuP (T_DECLARE)"
%token T_ENDDECLARE "HYYmcU (T_ENDDECLARE)"
%token T_AS         "pVBAWKL (T_AS)"
%token T_SWITCH     "BPDvsi (T_SWITCH)"
%token T_ENDSWITCH  "uxltGDKzxGejV (T_ENDSWITCH)"
%token T_CASE       "lglPLjqabJCxS (T_CASE)"
%token T_DEFAULT    "MUHxNkf (T_DEFAULT)"
%token T_BREAK      "dvQevQZzhOlI (T_BREAK)"
%token T_CONTINUE   "fvNBAJChu (T_CONTINUE)"
%token T_GOTO       "UZFHazdGBK (T_GOTO)"
%token T_FUNCTION   "FPiBsPAk (T_FUNCTION)"
%token T_CONST      "BrQERLXc (T_CONST)"
%token T_RETURN     "uAIfnkVzuK (T_RETURN)"
%token T_TRY        "LUhCjwHwaZxm (T_TRY)"
%token T_CATCH      "KDQUzTvZjoMNW (T_CATCH)"
%token T_FINALLY    "SwhXyycaPviSts (T_FINALLY)"
%token T_THROW      "KeBtPrfuxI (T_THROW)"
%token T_USE        "sbeHVVPzb (T_USE)"
%token T_INSTEADOF  "FMPzpm (T_INSTEADOF)"
%token T_GLOBAL     "sLjOTHfdUCfwA (T_GLOBAL)"
%token T_STATIC     "ukYsGzouGEqcP (T_STATIC)"
%token T_ABSTRACT   "dTzyIkQkifHJFQ (T_ABSTRACT)"
%token T_FINAL      "final (T_FINAL)"
%token T_PRIVATE    "XaasepVRm (T_PRIVATE)"
%token T_PROTECTED  "DiKXtbvnCMmAP (T_PROTECTED)"
%token T_PUBLIC     "lGKKDjTJugN (T_PUBLIC)"
%token T_VAR        "aRsjXuIlWYAGR (T_VAR)"
%token T_UNSET      "TFSlkXy (T_UNSET)"
%token T_ISSET      "AJOTyp (T_ISSET)"
%token T_EMPTY      "kSJLwHRyRBVZ (T_EMPTY)"
%token T_HALT_COMPILER "SSzAhnpOia (T_HALT_COMPILER)"
%token T_CLASS      "rflrrmVeZmD (T_CLASS)"
%token T_TRAIT      "kjmnwVHCUYPwPx (T_TRAIT)"
%token T_INTERFACE  "MoiwezTAK (T_INTERFACE)"
%token T_EXTENDS    "qtULKFtXSNPA (T_EXTENDS)"
%token T_IMPLEMENTS "SzebUMMvlSQMW (T_IMPLEMENTS)"
%token T_OBJECT_OPERATOR "-> (T_OBJECT_OPERATOR)"
%token T_DOUBLE_ARROW    "=> (T_DOUBLE_ARROW)"
%token T_LIST            "CgkxdLvwqMuwUK (T_LIST)"
%token T_ARRAY           "leKZdPRncy (T_ARRAY)"
%token T_CALLABLE        "QGDwiYSyzqLQ (T_CALLABLE)"
...
%token T_OPEN_TAG_WITH_ECHO "open tag with LMsvwqWMjeGR (T_OPEN_TAG_WITH_ECHO)"
...
%token T_NAMESPACE       "gxQCXoF (T_NAMESPACE)"
